...
Airbnb <- Airbnb %>%
  group_by(., GSS_CODE, NAME)%>%
  summarise(`Accomodation count` = unique(hotels_in_borough))
# at this point we have Airbnb sf object = airbnbs per borough; Hotels sf object = hotels per borough
#let's join them
all_accomodation <- st_join(Hotels, Airbnb)

head(all_accomodation)

#Join the same geometries
...
# trying different sigma value to visualise better
BluePlaquesSub.ppp %>%
  density(., sigma=1000) %>%
...
db
db$cluster
#adding back the cluster membership
BluePlaquesSubPoints<- BluePlaquesSubPoints %>%
  mutate(dbcluster=db$cluster)
#adding convext hull polygons
